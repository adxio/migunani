...
class m_upload extends CI_Model {
    ...
    protected $error_messages = array(
        ...
        'abort_db' => 'File uploaded successfull but Data cant insert to DB',
        'delete' => 'Data cant delete from DB'
    );
    ...
    public function get_options() {
        //get session
        $cat_id = !empty($_SESSION['uploaded']["cat_id"]) ? $_SESSION['uploaded']["cat_id"] : "";
        $project_id = !empty($_SESSION['uploaded']["project_id"]) ? $_SESSION['uploaded']["project_id"] : "";
        $max_file_size = !empty($_SESSION['uploaded']["max_file_size"]) ? $_SESSION['uploaded']["max_file_size"] : 0;
        $upload_dir = !empty($_SESSION['uploaded']["upload_dir"]) ? $_SESSION['uploaded']["upload_dir"] : "/";
        $delete_url = !empty($_SESSION['uploaded']["delete_url"]) ? $_SESSION['uploaded']["delete_url"] : "/";
        $upload_url = !empty($_SESSION['uploaded']["upload_url"]) ? $_SESSION['uploaded']["upload_url"] : "/";
        //--
        $options = array(
            'script_url' => $this->get_full_url() . '/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . $upload_dir,
            // 'delete_url' => $this->get_full_url().'/grad_upload/do_upload',
            'delete_url' => site_url($delete_url),
...
//            'accept_file_types' => '/.+$/i',
    ...
    protected function handle_form_data($file, $index) {
        ...
        $file->name = date('dmY') . "_" . rand(1000000, 9999999) . "." . end($ext);
        $file->cat_id = isset($_SESSION['uploaded']['cat_id']) ? $_SESSION['uploaded']['cat_id'] : '';
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if (empty($file->error)) {
            $params = array($_SESSION['uploaded']['cat_id'], $file->name, $_SESSION['uploaded']['user_id']);
            if (!$this->m_galery->insert($params)) {
                $file->error = $this->get_error_message('abort_db');
            } else {
                $file->id = $this->m_galery->get_last_inserted_id();
            }
        }
            ...
            if ($success) {
                foreach ($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
                        ...
                        //delete galery from db
                        $this->m_galery->delete(array($file_name, $_SESSION['uploaded']['cat_id']));
                        //
                        if (is_file($file)) {
                            unlink($file);
                        }
